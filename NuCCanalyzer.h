...
private:  
    ...
    std::string m_splittrack1_producer;
    std::string m_splittrack2_producer;
    ...
    trkf::TrackStatePropagator trkprop; 
    ...
    //For odd and even verticies
    std::vector<float> fv1pos; 
    std::vector<float> fv2pos;
    float fv1xpos;
    float fv2xpos;   
    float fv1ypos;
    float fv2ypos;
    ...
    std::vector<double> fSplitTrackMidPars;
    std::vector<double> fSplitTrack1Pars;
    std::vector<double> fSplitTrack2Pars;
    // Shower info
    float fShowerLength;
...
void NuCCanalyzer::reconfigure(fhicl::ParameterSet const &p)
{
    ...
    m_splittrack1_producer = p.get<std::string>("inputTracksLabel1st");
    m_splittrack2_producer = p.get<std::string>("inputTracksLabel2nd");
...
NuCCanalyzer::NuCCanalyzer(fhicl::ParameterSet const &p)
    :   
    EDAnalyzer(p), 
    mcsfitter(p.get<fhicl::ParameterSet>("mcsfitter")), 
...

{
    ...
    fEventTree->Branch("v1pos",&fv1pos); 
    fEventTree->Branch("v2pos",&fv2pos); 
    fEventTree->Branch("v1xpos",&fv1xpos);
    fEventTree->Branch("v2xpos",&fv2xpos);
    fEventTree->Branch("v1ypos",&fv1ypos);
    ...
    fNueDaughtersTree->Branch("strk_mid_gpar", &fSplitTrackMidPars);
    fNueDaughtersTree->Branch("strk_tk1_gpar", &fSplitTrack1Pars);
    fNueDaughtersTree->Branch("strk_tk2_gpar", &fSplitTrack2Pars);
...
void NuCCanalyzer::clearDaughter()
{
   ...
   ftk_vtx_gpar.clear();
    ...
    fSplitTrack2Pars.clear(); 

}

bool NuCCanalyzer::isFromNeutrino(art::ValidHandle<std::vector<recob::PFParticle> > pfph, size_t ipfp) {
